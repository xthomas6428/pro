...
class parser
{
    ...
    public function parseHtml($encoding = 'utf-8'): string
    {
        if ($this->html === '') {
            return '';
        }
        ...
        if ($node->hasChildNodes()) {
            foreach ($node->childNodes as $childNode) {
                if ($childNode instanceof DOMElement) {
                    if (!isset($this->elementsWhitelist[$childNode->tagName]) && !in_array($childNode->tagName,
                            ['html', 'body'])) {
     ...
     * @return Void
     */
    protected function removeDisallowedAttributes($node): void
    {
        if (isset($this->elementsWhitelist[$node->tagName]) && $node->hasAttributes()) {
            ...
            for ($i = 0; $i < $length; $i++) {
                ...
                if (!in_array($attrName, $elementAttributes, true)) {

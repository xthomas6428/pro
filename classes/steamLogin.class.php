...
class steamLogin
{
     ...
     * @return array
     */
    public static function getReturnToUrl(): array
    {
        global $using_ssl;
     ...
     */
    public static function genUrl($useAmp = true): string
    {
        [$ssl, $returnTo] = self::getReturnToUrl();
    ...
    {
        list($ssl, $returnTo) = self::getReturnToUrl();

        // Star off with some basic params
        if (isset($_GET['openid_assoc_handle'], $_GET['openid_signed'], $_GET['openid_sig'])) {
            $params = [
        ...
        // Return URL has to be on the same hostname
        $hostname = explode('/', str_replace(['https://', 'http://'], '', $returnTo))[0];
        if (!isset($_GET['openid_return_to']) || strpos($_GET['openid_return_to'], $hostname) === false) {
            return null;
        }

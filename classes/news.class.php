...
class news
{
    public static function del($p): void
    {
        global $db;
            ...
            if ($res !== null) {
                foreach ($res as $row) {
                    ...
                    if ($uid !== null && userExists($uid)) {
                        $pname = htmlspecialchars((string)$db->getOne("SELECT name FROM players WHERE uid = ?",
                            $uid));
                    }

                    $content = $db->getOne("SELECT LEFT(content, 200) AS content FROM news WHERE id = ?",
                        $id);
                    $content .= '... <a href="news.php?id='.$id.'">Read more</a>';
        ...
        foreach ($res as $row) {
            ...
            $ret = '
                <form method="POST">
                    ...
                    <textarea id="news_edit_textarea" name="news_edit_textarea" class="tinymce">'.$content.'</textarea>
                    <br>
    ...
    public static function getPostVal($id, $val)
    {
        ...
        return $db->getOne("SELECT $val FROM news WHERE id = ?", $id);
    }
}

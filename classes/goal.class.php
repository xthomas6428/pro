...
class goal
{
    ...
    public static function get(): array
    {
        ...
        $day = (int) getSetting('enable_goal', 'value');
        if (empty($day) || !is_numeric($day)) {
            $day = 1;
        }

        if ($day !== 0 && strpos($day, '0') !== false) {
            $day = substr($day, 1);
        }
        ...
        if ($day <= date('d')) {
            # This means we have to start from scratch, probably go one month into the future as well lol

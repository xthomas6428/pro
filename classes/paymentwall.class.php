...
class paymentwall
{
    public static function displayWidget($item_id, $uid, $type): string
    {
        global $db;
        ...
        $num_delivered = $db->getOne("SELECT count(*) AS value FROM transactions WHERE uid = ? AND email != 'Assigned by Admin' AND gateway = 'paymentwall'",
            $uid);
